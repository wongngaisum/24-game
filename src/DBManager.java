...
public class DBManager {
	...
	private void initialize() throws SQLException {
		synchronized (userInfoLock) {
			...
			stmt.execute(
					"CREATE TABLE IF NOT EXISTS UserInfo (username VARCHAR(32) NOT NULL, password VARCHAR(32) NOT NULL, wins INT NOT NULL, "
		...
		synchronized (onlineUserLock) {
			Statement stmt = conn.createStatement();
			stmt.executeUpdate(
					"CREATE TABLE IF NOT EXISTS OnlineUser (username VARCHAR(32) NOT NULL, PRIMARY KEY(username))");
		}
	...
	public String register(String username, String password) throws SQLException {
		synchronized (userInfoLock) {
			PreparedStatement stmt = conn.prepareStatement(
					"INSERT INTO UserInfo (username, password, wins, games_played, average_time) VALUES (?, ?, ?, ?, ?)");
			stmt.setString(1, username);
	...
	public void updateWinnerInfo(GameRoom room, User winner) throws SQLException {
		synchronized (userInfoLock) {
			for (int i = 0; i < room.getPlayers().size(); i++) {
				PreparedStatement stmt1 = conn
						.prepareStatement("SELECT wins, games_played, average_time FROM UserInfo WHERE username = ?");
				stmt1.setString(1, room.getPlayers().get(i).getUsername());
				ResultSet rs = stmt1.executeQuery();

				if (rs.next()) {
					int noOfWins = rs.getInt(1);
					int noOfGames = rs.getInt(2) + 1;
					double averageTime = rs.getDouble(3);

					if (room.getPlayers().get(i).equals(winner)) {
						noOfWins++;
						averageTime = Double.parseDouble(new DecimalFormat(".##")
								.format((averageTime * (noOfWins - 1) + room.getTimeToWin()) / noOfWins));
					}

					PreparedStatement stmt2 = conn.prepareStatement(
							"UPDATE UserInfo SET wins = ?, games_played = ?, average_time = ? WHERE username = ?");
					stmt2.setInt(1, noOfWins);
					stmt2.setInt(2, noOfGames);
					stmt2.setDouble(3, averageTime);
					stmt2.setString(4, room.getPlayers().get(i).getUsername());
